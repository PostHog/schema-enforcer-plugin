...
const areValuesValid = (jsonSchema: SchemaObject) => {
  try {
    for (const eventSchema of Object.values(jsonSchema.eventSchemas)) {
      if (
        eventSchema.acceptOnlySchemaProps &&
        !isBool(eventSchema.acceptOnlySchemaProps)
      ) {
        return false;
      }
      ...
      for (const prop of Object.values(eventSchema.schema)) {
        if (
          (prop.required && !isBool(prop.required)) ||
          !ACCEPTED_TYPES.has(prop.type)
        ) {
          return false;
        }
      }
  ...
  const checks = [
    {
      ...
      errorMessage:
        "Your file has no specified schemas. Please specify at least one.",
    },
    {
